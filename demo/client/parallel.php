...
class ParallelClient extends Client
{
    public function sendParallel($requests, $timeout = 0, $method = '')
    {
        ...
        foreach($responses as $k => $resp) {
            if (!$resp) {
                $responses[$k] = new static::$responseClass(0, PhpXmlRpc::$xmlrpcerr['curl_fail'], PhpXmlRpc::$xmlrpcstr['curl_fail'] . ': ' . $errors[$k]);
            } else {
...
// using parallel http requests, and using a single batch request
...
$client = new ParallelClient(JSONRPCSERVER);

// avoid storing http info in the responses, to make the checksums comparable
$client->setDebug(-1);

// make client use json-rpc 2.0
echo "Making $num_tests calls to method interopEchoTests.echoValue on server " . JSONRPCSERVER . " ...\n";
flush();
...
if (strpos(JSONRPCSERVER, 'http://') === 0) {
