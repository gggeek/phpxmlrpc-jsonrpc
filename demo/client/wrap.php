...
<h3>The code demonstrates usage of some of the most automagic client usage possible:<br/>
    1) client that returns php values instead of json-rpc Value objects<br/>
...
$client = new PhpXmlRpc\JsonRpc\Client(JSONRPCSERVER);
$client->setOption(\PhpXmlRpc\Client::OPT_RETURN_TYPE, 'phpvals'); // let client give us back php values instead of xmlrpcvals
$resp = $client->send(new PhpXmlRpc\JsonRpc\Request('system.listMethods'));
if ($resp->faultCode()) {
