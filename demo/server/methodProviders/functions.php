 ...
 *
 * Demoes a simple possible way to implement webservices without cluttering the global scope: create json-rpc-aware static
 ...
 * 2. define functions in the global scope to be used as json-rpc method handlers: see interop.php
 * 3. define json-rpc method handlers as anonymous functions directly within the dispatch map: see validator1.php
 * 4. use php methods or functions which are not aware of json-rpc and let the Server do all the necessary type conversion:
 *    see discuss.php
 * 5. use the JsonRpc\Wrapper class to achieve the same as in point 4, with no need to manually write the dispatch map
 *    configuration (but taking instead a performance hit)
 * 6. use the JsonRpc\Wrapper class to generate php code in offline mode, achieving the same as in point 5 with no
