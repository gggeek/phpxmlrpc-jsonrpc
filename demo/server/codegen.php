...
/**
 * Demoing the code-generation capabilities of the library: create all that is required to expose as json-rpc methods
 * a bunch of methods of an instance of a php class which is totally unaware of json-rpc.
 */
...
if (isset($_GET['generate']) && $_GET['generate']) {
    ...
    // CommentManager is the "json-rpc-unaware" class, whose methods we want to make accessible via json-rpc calls
    $cm = new CommentManager(sys_get_temp_dir() . "/comments.db");

    // analyze the CommentManager instance and generate both code defining stub-methods and a dispatch map for the json-rpc Server
    $w = new Wrapper();
    $code = $w->wrapPhpClass(
        $cm,
        array(
    ...
    // generate separate files with the json-rpc server instantiation and its dispatch map
    ...
    file_put_contents($targetControllerFile,
        ...
        "\PhpXmlRpc\JsonRpc\Wrapper::holdObject('*', \$cm);\n\n" .
        ...
        '$s = new \PhpXmlRpc\JsonRpc\Server($dm, false);' . "\n\n" .
    ...
    // test that everything worked by running it in realtime (note that this script will return a json-rpc error message if
    // run from the command line, as the server will find no json-rpc payload to operate on)
