...
class Client extends BaseClient
{
    ...
    public function send($req, $timeout = 0, $method = '')
    {
        ...
        // inject the req. Id and jsonrpc version. The id mapping works in case the requests are not sent in a batch call,
        // as we are guaranteed to receive an array of responses in the same order. For batch calls, either the main
        // request is ko, or all the "subrequests" are bound to be created from the server response, and this loop does nothing
        if (is_array($req)) {
            foreach ($req as $i => $r) {
                /// @todo test: does this work when notifications are mixed with calls?
                if (!is_string($r)) {
                    if (!is_bool($resp[$i]) && !$resp[$i]->isFromServer()) {
                        if ($r->id() !== null) {
            ...
            if (!is_bool($resp) && !$resp->isFromServer()) {
                if ($req->id() !== null) {
    ...
    protected function _try_multicall($reqs, $timeout, $method)
    {
        ...
        if ($this->return_type == 'xml' || $this->return_type == 'json') {
            for ($i = 0; $i < count($reqs); $i++) {
                /// @todo can we do better? We set the complete json into each response... Note that we could f.e.
                ///       use the req Id as resp Id - but nothing in the spec says resp order must match reqs order
                $response[$i] = new static::$responseClass($rets, 0, '', 'json', null, $result->httpResponse());
                $response[$i]->setIsFromServer(true);
            }
            ...
            for ($i = 0; $i < $numRets; $i++) {
                ...
                if (array_key_exists('result', $val)) {
                    // Normal return value
                    $response[$i] = new static::$responseClass($val['result'], 0, '', 'phpvals', $respId, $result->httpResponse());
                    ...
                    $response[$i] = new static::$responseClass(0, $code, $str, 'phpvals', $respId, $result->httpResponse());
                } else {
                    return new static::$responseClass(0, PhpXmlRpc::$xmlrpcerr['multicall_error'],
                        PhpXmlRpc::$xmlrpcstr['multicall_error'] . ": response element $i has neither result nor error",
                        'phpvals', null, $result->httpResponse());
                }
                if (array_key_exists('jsonrpc', $val)) {
                    $response[$i]->setJsonRpcVersion($val['jsonrpc']);
                } else {
                    $response[$i]->setJsonRpcVersion(PhpJsonRpc::VERSION_1_0);
                }
                $response[$i]->setIsFromServer(true);
            }
            ...
            foreach ($rets as $i => $val) {
                if ($val->kindOf() == 'struct') {
                    $respId = null;
                    if (isset($val['id'])) {
                        $respId = $val['id']->scalarVal();
                    }
                    if (isset($val['result'])) {
                        $response[$i] = new static::$responseClass($val['result'], 0, '', 'jsonrpcvals', $respId, $result->httpResponse());
                        ...
                        $response[$i] = new static::$responseClass(null, $code->scalarval(), $str->scalarval(),
                            'jsonrpcvals', $respId, $result->httpResponse());
                    } else {
                        return new static::$responseClass(0, PhpXmlRpc::$xmlrpcerr['multicall_error'],
                            PhpXmlRpc::$xmlrpcstr['multicall_error'] . ": response element $i has neither result nor error",
                            'phpvals', null, $result->httpResponse());
                    }
                    if (isset($val['jsonrpc'])) {
                        $response[$i]->setjsonRpcVersion($val['jsonrpc']->scalarVal());
                    } else {
                        $response[$i]->setJsonRpcVersion(PhpJsonRpc::VERSION_1_0);
                    }
                    $response[$i]->setIsFromServer(true);
